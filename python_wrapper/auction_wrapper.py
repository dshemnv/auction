...
def convert_assignment_result_to_np(assignment_result, shape):
    return np.ctypeslib.as_array(assignment_result.contents.agent_to_object, shape=shape)
...
class CAuctionWrapper:
    ...
    def solve_auction(self, cost_matrix: np.ndarray, eps: float=0.01 ):
        ...
        return convert_assignment_result_to_np(result, (cols,)), t
...
if __name__ == "__main__":
    ...
    out, c_auction_t = c_auction.solve_auction(test_mat, 0.01)
    print("[Py]: Solver finished")
    t1 = time.perf_counter()
    _, res = linear_sum_assignment(-test_mat)
